...
class GeocodedPlaces:
    ...
    def GET(self, callback="", *arg, **kwargs):
        sql = "SELECT (place, latitude, longitude) FROM geocode "
        conn = connectToDatabase("localhost", "scottsfarley", 'Sequoia93!', 'WoodenShips')
        cursor = conn.cursor()
        cursor.execute(sql)
        ...
        for row in rows:
            ...
            try:
                if float(row[1]) != 0 and float(row[2]) != 0 and row[0] != "":
...
class RawData:
    ...
    def GET(self, header=False, logbookLanguage = "", voyageFrom = "", voyageTo = "", shipName = "", shipType = "", company = "", nationality = "",
        ...
        print "Wind Speed is: " , windSpeedMin
        conn = connectToDatabase("localhost", "scottsfarley", 'Sequoia93!', 'WoodenShips')
        cursor = conn.cursor()
        sql = "SELECT "
        if fieldlist != "":
        ...
        if voyageFrom != "":
            sql += " voyagefrom='" + str(voyageFrom) + "' AND "
        if voyageTo != "":
            sql += " voyageto='" + str(voyageTo) + "' AND "
        if shipName != "":
